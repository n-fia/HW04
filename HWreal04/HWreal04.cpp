...
public:
    ...
    //책이 추가될 때 책의 권수를 초기화하기 > main함수에서 사용
    ...
    void displayStock() {
        ...
        for (const auto& sto : stock) {
            cout << "책 제목 : " << sto.first << "    재고 : " << sto.second << "권" << endl;
        }
    }
};
...
int main() {
    ...
    manager.addBook("1번 도서", "1번 저자"); //그냥 string&이면 수정 가능한 객체를 넣어줘야 해서 문자열을 바로 못 넣는데, const string& str이니까 수정 안 되고 컴파일러가 알아서 임시적 객체를 생성
    ...
    while (true) {
        ...
        std::cout << "6. 도서 대여 현황" << std::endl; // 대여 현황
        std::cout << "선택: ";
